...
def get_dashboard_stats():
    ...
    try:
        ...
        if user_role == 'admin':
            pending_students = execute_db("SELECT COUNT(*) as count FROM students WHERE status = 0")
            pending_teachers = execute_db("SELECT COUNT(*) as count FROM teachers WHERE status = 0") 
            pending_admins = execute_db("SELECT COUNT(*) as count FROM admin WHERE status = 0")
            pending_count = pending_students[0]['count'] + pending_teachers[0]['count'] + pending_admins[0]['count']
        elif user_role == 'teacher':
            pending_students = execute_db("SELECT COUNT(*) as count FROM students WHERE status = 0")
            pending_count = pending_students[0]['count']
            
        unread_messages_result = execute_db("""
            SELECT COUNT(*) as count FROM messages 
            WHERE recipient_id = ? AND recipient_role = ? AND is_read = 0
        """, user_id, user_role)
        unread_messages = unread_messages_result[0]['count']
...
def get_pending_accounts_count(user_role):
    ...
    try:
        if user_role == 'admin':
            student_count = execute_db("SELECT COUNT(*) as count FROM students WHERE status = 0")[0]['count']
            teacher_count = execute_db("SELECT COUNT(*) as count FROM teachers WHERE status = 0")[0]['count']
            admin_count = execute_db("SELECT COUNT(*) as count FROM admin WHERE status = 0")[0]['count']
            count = student_count + teacher_count + admin_count
        elif user_role == 'teacher':
            count = execute_db("SELECT COUNT(*) as count FROM students WHERE status = 0")[0]['count']
    except:
        count = 0
    return count
...
def login_post():
    try:
        ...
        user = None
        if role == 'student':
            users = execute_db("SELECT * FROM students WHERE email = ?", email)
            ...
            users = execute_db("SELECT * FROM teachers WHERE email = ?", email)
            if users and check_password_hash(users[0]['password'], password):
                if users[0]['status'] == 0:
            ...
            users = execute_db("SELECT * FROM admin WHERE email = ?", email)
            if users and check_password_hash(users[0]['password'], password):
                if users[0]['status'] == 0:
...
def register():
    ...
    if role == 'student':
        try:
            courses = execute_db("SELECT * FROM courses")
        except:
            courses = []
...
def register_post():
    try:
        ...
        existing_users = []
        try:
            existing_users.extend(execute_db("SELECT email FROM students WHERE email = ?", email))
            existing_users.extend(execute_db("SELECT email FROM teachers WHERE email = ?", email))
            existing_users.extend(execute_db("SELECT email FROM admin WHERE email = ?", email))
        except:
            pass
...
def dashboard():
    ...
    try:
        if user_role == 'admin':
            stats = {
                'total_students': execute_db("SELECT COUNT(*) as count FROM students WHERE status = 1")[0]['count'],
                'total_teachers': execute_db("SELECT COUNT(*) as count FROM teachers WHERE status = 1")[0]['count'],
                'total_courses': execute_db("SELECT COUNT(*) as count FROM courses")[0]['count'],
                'pending_approvals': get_dashboard_stats()['pending_accounts_count'],
                'unpaid_fees': execute_db("SELECT COUNT(*) as count FROM students WHERE fee_status = 'unpaid' AND status = 1")[0]['count']
            }
        elif user_role == 'teacher':
            stats = {
                'my_students': execute_db("SELECT COUNT(*) as count FROM students WHERE status = 1")[0]['count'],
                'my_courses': execute_db("SELECT COUNT(*) as count FROM courses WHERE teacher_id = ?", user_id)[0]['count'],
                'assessments': execute_db("SELECT COUNT(*) as count FROM assessments WHERE teacher_id = ?", user_id)[0]['count'],
                'pending_approvals': get_dashboard_stats()['pending_accounts_count']
            }
        elif user_role == 'student':
            attendance_data = execute_db("""
            ...
            fee_status = execute_db("SELECT fee_status FROM students WHERE student_id = ?", user_id)
...
def compose_message():
    ...
    try:
        recipients = []
        if user_role == 'admin':
            teachers = execute_db("SELECT teacher_id as id, name, 'teacher' as role FROM teachers WHERE status = 1")
            students = execute_db("SELECT student_id as id, name, 'student' as role FROM students WHERE status = 1")
            recipients = teachers + students
        elif user_role == 'teacher':
            admins = execute_db("SELECT admin_id as id, name, 'admin' as role FROM admin WHERE status = 1")
            teachers = execute_db("SELECT teacher_id as id, name, 'teacher' as role FROM teachers WHERE status = 1 AND teacher_id != ?", session.get("user_id"))
            students = execute_db("SELECT student_id as id, name, 'student' as role FROM students WHERE status = 1")
...
def manage_students():
    ...
    try:
        ...
        students = execute_db(query, *params)
...
def student_details(student_id):
    ...
    try:
        ...
        student = execute_db("SELECT * FROM students WHERE student_id = ?", student_id)
        if not student:
        ...
        # Get course results
        course_results = execute_db("""
            SELECT * FROM course_results WHERE student_id = ?
        """, student_id)

        # Get assessment details
        assessment_details = execute_db("""
...
def update_student(student_id):
    ...
    try:
        ...
        existing = execute_db("SELECT * FROM students WHERE email = ? AND student_id != ?", email, student_id)
        if existing:
...
def toggle_student_status(student_id):
    ...
    try:
        ...
        execute_db_commit("UPDATE students SET status = ? WHERE student_id = ?", new_status, student_id)
...
def delete_student(student_id):
    ...
    try:
        ...
        attendance_records = execute_db("SELECT COUNT(*) as count FROM attendance WHERE student_id = ?", student_id)
        ...
        grade_records = execute_db("SELECT COUNT(*) as count FROM grades WHERE student_id = ?", student_id)
        if grade_records[0]['count'] > 0:
            flash("Cannot delete student with assessment results. Please deactivate instead.")
            return redirect(url_for('manage_students'))

        # Delete the student
        execute_db_commit("DELETE FROM students WHERE student_id = ?", student_id)
...
def manage_courses():
    ...
    try:
        ...
        teachers = execute_db("SELECT teacher_id, name FROM teachers WHERE status = 1")
...
def add_course():
    ...
    try:
        ...
        existing = execute_db("SELECT * FROM courses WHERE course_name = ?", course_name)
        if existing:
...
def delete_course(course_id):
    """Delete course"""
    try:
        execute_db_commit("DELETE FROM courses WHERE course_id = ?", course_id)
...
def assign_teacher(course_id):
    ...
    try:
        ...
        if teacher_id:
            execute_db_commit("UPDATE courses SET teacher_id = ? WHERE course_id = ?", int(teacher_id), course_id)
            flash("Teacher assigned successfully!")
        else:
            execute_db_commit("UPDATE courses SET teacher_id = NULL WHERE course_id = ?", course_id)
...
def teacher_details(teacher_id):
    ...
    try:
        ...
        teacher = execute_db("SELECT * FROM teachers WHERE teacher_id = ?", teacher_id)
        if not teacher:
...
def toggle_teacher_status(teacher_id):
    ...
    try:
        ...
        execute_db_commit("UPDATE teachers SET status = ? WHERE teacher_id = ?", new_status, teacher_id)
...
def delete_teacher(teacher_id):
    ...
    try:
        ...
        assigned_courses = execute_db("SELECT COUNT(*) as count FROM courses WHERE teacher_id = ?", teacher_id)
        ...
        created_assessments = execute_db("SELECT COUNT(*) as count FROM assessments WHERE teacher_id = ?", teacher_id)
        if created_assessments[0]['count'] > 0:
            flash("Cannot delete teacher with created assessments. Please reassign assessments first.")
            return redirect(url_for('manage_teachers'))

        # Delete the teacher
        execute_db_commit("DELETE FROM teachers WHERE teacher_id = ?", teacher_id)
...
def create_assessment():
    try:
        courses = execute_db("SELECT * FROM courses ORDER BY course_name")
    except:
        courses = []

    assessment_types = ['Slip test', 'Assignment', 'Midterm', 'Final Exam']
...
@teacher_or_admin_required
def handle_create_assessment():
    try:
        user_id = session.get("user_id")
        form_data = {
            "title": request.form.get('assessment_name', '').strip(),
            "course_name": request.form.get('course_name', '').strip(),
            "assessment_type": request.form.get('assessment_type', '').strip(),
            "max_marks": request.form.get('total_marks', '').strip(),
        ...
        try:
            form_data["max_marks"] = float(form_data["max_marks"])
            form_data["weightage"] = float(form_data["weightage"])
        except ValueError:
            flash("Total Marks and Weightage must be numbers")
            return redirect(url_for('create_assessment'))

        # Insert into database
        execute_db_commit("""
            INSERT INTO assessments 
            (assessment_name, course_name, assessment_type, total_marks, weightage, assessment_date, teacher_id, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'active')
        """, form_data["title"], form_data["course_name"], form_data["assessment_type"],
             form_data["max_marks"], form_data["weightage"], form_data["assessment_date"], user_id)
...
def grade_assessment(assessment_id):
    ...
    try:
        assessment = execute_db("SELECT * FROM assessments WHERE assessment_id = ?", assessment_id)[0]
    ...
    try:
        for key, value in request.form.items():
            if key.startswith('marks_'):
                ...
                student = execute_db("SELECT course FROM students WHERE student_id = ?", student_id)[0]
...
def student_results():
    ...
    try:
        results = execute_db("""
            SELECT cr.*, a.assessment_name, a.assessment_type, a.total_marks, g.obtained_marks
...
def attendance_report():
    ...
    try:
        if user_role == 'student':
            ...
            attendance_data = execute_db(query, *params)
...
def update_fee_status(student_id):
    ...
    try:
        ...
        execute_db_commit("UPDATE students SET fee_status = ? WHERE student_id = ?", new_status, student_id)

        if new_status == 'paid':
            student = execute_db("SELECT s.name, c.fee FROM students s LEFT JOIN courses c ON s.course = c.course_name WHERE s.student_id = ?", student_id)[0]
            if student['fee']:
                execute_db_commit("""
...
def finance():
    ...
    try:
        transactions = execute_db(query, *params)
...
def mark_notification_read(notification_id):
    try:
        ...
        notification = execute_db("SELECT read_by FROM notifications WHERE notification_id = ?", notification_id)
...
def delete_notification(notification_id):
    ...
    try:
        execute_db_commit("DELETE FROM notifications WHERE notification_id = ?", notification_id)
...
def delete_all_notifications():
    ...
    try:
        execute_db_commit("DELETE FROM notifications")
...
def pending_accounts():
    ...
    try:
        if user_role == 'admin':
            pending_students = execute_db("SELECT * FROM students WHERE status = 0 ORDER BY student_id DESC")
            pending_teachers = execute_db("SELECT * FROM teachers WHERE status = 0 ORDER BY teacher_id DESC")
            pending_admins = execute_db("SELECT * FROM admin WHERE status = 0 ORDER BY admin_id DESC")
        elif user_role == 'teacher':
            pending_students = execute_db("SELECT * FROM students WHERE status = 0 ORDER BY student_id DESC")
    except Exception as e:
...
@app.route('/approve_account/<string:role>/<int:account_id>', methods=['POST'])
@teacher_or_admin_required
def approve_account(role, account_id):
    try:
        ...
        if role == 'student':
            execute_db_commit("UPDATE students SET status = 1 WHERE student_id = ?", account_id)
            flash("Student account approved successfully!")
        elif role == 'teacher':
            execute_db_commit("UPDATE teachers SET status = 1 WHERE teacher_id = ?", account_id)
            flash("Teacher account approved successfully!")
        elif role == 'admin':
            execute_db_commit("UPDATE admin SET status = 1 WHERE admin_id = ?", account_id)
...
@app.route('/reject_account/<string:role>/<int:account_id>', methods=['POST'])
@teacher_or_admin_required
def reject_account(role, account_id):
    try:
        ...
        if role == 'student':
            execute_db_commit("DELETE FROM students WHERE student_id = ?", account_id)
            flash("Student account rejected and removed.")
        elif role == 'teacher':
            execute_db_commit("DELETE FROM teachers WHERE teacher_id = ?", account_id)
            flash("Teacher account rejected and removed.")
        elif role == 'admin':
            execute_db_commit("DELETE FROM admin WHERE admin_id = ?", account_id)
